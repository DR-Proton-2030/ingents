...
const handleGoogleSignIn = async () => {
  ...
  try {
    ...
    if (!response.ok) {
      setApiError(data.message || "Google sign-in failed");
      return;
    }
    ...
    setApiError("Something went wrong with Google sign-in. Please try again.");
  } finally {
    setIsLoading(false);
  }
};
    ...
    {isLoading ? (
      ...
      "Sign In"
          ...
          <button
            ...
            disabled={isLoading}
            className="w-full mt-6 flex items-center justify-center gap-3 px-8 py-3 rounded-full font-medium bg-white/80 border border-gray-300 hover:border-gray-400 hover:bg-white transition-all text-gray-700 disabled:opacity-50 disabled:cursor-not-allowed"
          >
