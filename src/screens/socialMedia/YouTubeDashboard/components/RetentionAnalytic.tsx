...
const RetentionAnalytic = ({ retention }: RetentionAnalyticProps) => {
  ...
  const topVideos = retention?.topVideos || [];
  ...
  return (
    <div className="space-y-6">
      ...
      {topVideos.length > 0 && (
        <div className="bg-white p-8 rounded-[40px] shadow-sm border border-gray-100">
          <h3 className="text-xl font-black text-slate-900 mb-6">Top Video Retention</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {topVideos.map((video: any, idx: number) => (
              <div key={idx} className="p-5 rounded-3xl bg-slate-50 border border-slate-100 space-y-3 hover:bg-white hover:shadow-md transition-all">
                <div className="flex justify-between items-start">
                  <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Video ID: {video.videoId}</span>
