...
const XStatsDashboard = () => {
  ...
  const handleDisconnect = async () => {
    ...
    if (!xConnected) {
      toast.info("X is already disconnected");
      return;
    }
    ...
    const confirmed =
      typeof window !== "undefined"
        ? window.confirm(
            "Disconnect X? You can reconnect anytime from Social Media.",
          )
        : true;
    if (!confirmed) return;
    ...
    try {
      ...
      if (payload?.user) {
        ...
        setUser({
          ...(user as any),
          x: { project_id: null, name: null, access_token: null },
        });
      }
      ...
      toast.error(err?.message || "Failed to disconnect X");
    } finally {
      ...
      <Layout showSidebar={true}>
        <div className="min-h-screen bg-[#EAEEF6] flex flex-col items-center justify-center p-8">
          <div className="bg-white p-8 rounded-[32px] shadow-sm text-center max-w-md">
            ...
            <p className="text-gray-600 mb-8">{error || "Failed to load X profile data. Please ensure your account is connected."}</p>
            <Link 
              href="/site/social-media" 
              className="bg-black text-white px-8 py-3 rounded-xl font-bold hover:bg-gray-900 transition-colors inline-block"
            >
              Go Back
            </Link>
          </div>
      ...
      <div className="min-h-screen bg-[#EAEEF6] p-4 lg:p-8 font-sans">
        <div className="max-w-[1400px] mx-auto space-y-8">
          <div className="flex justify-between items-center">
             ...
             <div className="flex items-center gap-3">
               <button
                 ...
                 disabled={!xConnected || isDisconnecting}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
            ...
            {/* Middle Column */}
            <div className="lg:col-span-4 space-y-6">
              <PostActivity activity={data?.postActivity} platform="x" />
              <AnomalyCard statistics={data?.page} platform="x" />
            </div>

            {/* Right Column */}
            <div className="lg:col-span-4 space-y-6">
              <PostSchedule schedule={data?.postSchedule} />
              <PostInsights posts={data?.recentPosts} platform="x" />
            </div>
...
export default XStatsDashboard;
