    ...
    try {
      ...
      if (isScheduled) {
        ...
        if (selectedPlatforms.includes("youtube")) {
          ...
          if (!video.url) {
            toast.warning(
              "Manual file upload for YouTube is being processed. (Requires S3 URL)",
            );
          }
        ...
        if (selectedPlatforms.includes("instagram")) {
          ...
          if (showScheduler && scheduleDate && scheduleTime) {
            ...
            igFormData.append("scheduleAt", scheduledDateTime);
          }

          await postInstagramContent(igFormData);
          toast.success(
            showScheduler
              ? "Post scheduled for Instagram!"
              : "Posted to Instagram!",
          );
        }
          ...
          >
            ...
            <div className="px-6 py-4 border-t border-slate-100 bg-gradient-to-r from-slate-50 to-white flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <button
                  onClick={() =>
                    document.getElementById("hashtag-input-field")?.focus()
                  }
                  className="flex items-center gap-2 px-4 py-2.5 bg-white border-2 border-slate-200 rounded-xl text-slate-600 text-xs font-semibold hover:bg-slate-50 hover:border-slate-300 transition-all shadow-sm"
                >
                  <FiHash size={14} />
          ...
          >
            ...
            <div className="flex justify-center mb-6">
              <div className="flex bg-slate-100 rounded-2xl p-1.5 gap-1">
                {(
                  ["instagram", "facebook", "x", "youtube"] as PreviewPlatform[]
                ).map((platform) => {
                  const config =
                    platform === "x"
                      ? platformIcons["X"]
