  ...
  return (
    ...
    >
      <div className="flex justify-between items-start mb-6">
        <div
          className={`p-4 rounded-2xl transition-all duration-500 shadow-sm ${variant === "default" ? "bg-white/80 text-blue-600 group-hover:bg-[#1877F2] group-hover:text-white" : "bg-white/80 text-slate-900 group-hover:scale-110"}`}
        >
          {icon}
        </div>
        {tooltipText && (
      ...
      <div>
        <p
          className={`text-[10px] font-black uppercase tracking-[0.2em] mb-2 ${variant === "default" ? "text-slate-400" : "text-slate-500 opacity-70"}`}
        >
          {title}
        </p>
        <div className="flex items-baseline gap-3">
          ...
          {trend && !disabled && (
            <div
              className={`flex items-center gap-1.5 px-3 py-1 rounded-full text-[10px] font-black ${trend.isPositive ? "bg-emerald-400/20 text-emerald-700 shadow-sm shadow-emerald-500/10" : "bg-red-400/20 text-red-700 shadow-sm shadow-red-500/10"}`}
            >
              {trend.isPositive ? "+" : "-"}
              {trend.value}%
            </div>
