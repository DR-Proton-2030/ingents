...
const FBContentList: React.FC<FBContentListProps> = ({ content, title, layout = "list" }) => {
  if (!content || content.length === 0) {
    return (
      <div className="bg-white/60 p-16 rounded-[40px] border border-dashed border-white/60 backdrop-blur-xl text-center">
        ...
        <p className="text-slate-400 font-black text-xs uppercase tracking-widest text-shadow-sm">Digital Void: No Content Detected</p>
      </div>
    );
  }
    ...
    return (
      <div className="space-y-8">
        <div className="flex items-center justify-between px-2">
            ...
            <span className="text-[10px] font-black text-blue-600 bg-blue-50 px-4 py-1.5 rounded-full uppercase tracking-widest border border-blue-100 italic">Official Published Log</span>
        </div>
          ...
          {content.map((item, idx) => (
            ...
            >
              <div className="relative aspect-video overflow-hidden bg-white/20">
                <img
                  src={item.thumbnail || "https://picsum.photos/seed/placeholder/400/225"}
                  alt={item.title}
              ...
              <div className="p-6">
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[11px] font-black text-slate-700">{formatCompactNumber(item.views)}</span>
                  </div>
        ...
        {content.map((item, idx) => (
          ...
          >
            <div className="relative w-16 h-16 rounded-2xl overflow-hidden bg-white/20 flex-shrink-0 shadow-sm border border-white/20">
                <img 
                  src={item.thumbnail || "https://picsum.photos/seed/placeholder/100/100"} 
                  alt="" 
                  className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500" 
                />
                <div className="absolute top-1 left-1 bg-blue-600 text-white w-5 h-5 rounded-lg flex items-center justify-center text-[8px] font-black">{idx + 1}</div>
            </div>
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-3">
                 <div className="flex items-center gap-1.5 px-2 py-0.5 rounded-lg bg-blue-50 text-blue-600">
                    ...
                    <span className="text-[10px] font-black">{formatCompactNumber(item.views)}</span>
                 </div>
                 {item.watchTime && (
